/*
 * MB8ARTModbus.cpp - part of the ESP32-MB8ART library
 *
...
void MB8ART::handleSensorError(int sensorIndex, char* statusBuffer, size_t bufferSize, int& offset) {
    ...
    // Propagate the fault to the bound validity pointer so consumers
    // (e.g. SharedSensorReadings.isBoilerTempOutputValid) see the channel as
    // invalid instead of retaining the last "valid" reading. Mirrors the
    // out-of-range branch in updateSensorReading(). A device sensor-fault code
    // (0x7530, open circuit / broken PT1000) routes here and previously left
    // the bound flag stuck true with a frozen temperature. (Audit F6)
    if (sensorIndex >= 0 && sensorIndex < DEFAULT_NUMBER_OF_SENSORS &&
        sensorBindings[sensorIndex].validityPtr != nullptr) {
        *sensorBindings[sensorIndex].validityPtr = false;
    }
