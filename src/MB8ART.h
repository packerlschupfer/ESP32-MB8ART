...
// Define an enum class for sensor update event bits
enum class SensorUpdateBits {
    SENSOR1_UPDATE_BIT = (1 << 0),
    SENSOR2_UPDATE_BIT = (1 << 1),
    SENSOR3_UPDATE_BIT = (1 << 2),
    SENSOR4_UPDATE_BIT = (1 << 3),
    SENSOR5_UPDATE_BIT = (1 << 4),
    SENSOR6_UPDATE_BIT = (1 << 5),
    SENSOR7_UPDATE_BIT = (1 << 6),
    SENSOR8_UPDATE_BIT = (1 << 7)
};

// Define an enum class for sensor error event bits
enum class SensorErrorBits {
    SENSOR1_ERROR_BIT = (1 << 0),
    SENSOR2_ERROR_BIT = (1 << 1),
    SENSOR3_ERROR_BIT = (1 << 2),
    SENSOR4_ERROR_BIT = (1 << 3),
    SENSOR5_ERROR_BIT = (1 << 4),
    SENSOR6_ERROR_BIT = (1 << 5),
    SENSOR7_ERROR_BIT = (1 << 6),
    SENSOR8_ERROR_BIT = (1 << 7)
};
...
inline EventBits_t toUnderlyingType(SensorErrorBits bit) {
    return static_cast<EventBits_t>(bit);
}

// Helper function to get update/error bit for a specific sensor index
inline SensorUpdateBits getSensorUpdateBit(size_t sensorIndex) {
    return static_cast<SensorUpdateBits>(1 << sensorIndex);
}
...
public:
    ...
    // Get the update event group for monitoring temperature updates
    EventGroupHandle_t getUpdateEventGroup() const noexcept { return xUpdateEventGroup; }
    ...
    void clearAllErrorBits() {
        xEventGroupClearBits(xErrorEventGroup, 0x00FFFFFF);  // Clear all 24 bits
    }
...
private:
    ...
    EventGroupHandle_t xUpdateEventGroup;  // Sensor update bits (direct access, no shifting)
    EventGroupHandle_t xErrorEventGroup;   // Sensor error bits (direct access, no shifting)
    
    // Separate event group for initialization bits
