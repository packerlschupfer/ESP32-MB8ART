...
### Required
- [esp32ModbusRTU](https://github.com/packerlschupfer/esp32ModbusRTU) - Modbus RTU implementation (maintained fork; upstream bertmelis repo is unmaintained since 2021)
- [ESP32-ModbusDevice](https://github.com/packerlschupfer/ESP32-ModbusDevice) - Modbus device framework
- [ESP32-MutexGuard](https://github.com/packerlschupfer/ESP32-MutexGuard) - Thread-safe mutex wrapper
- [ESP32-IDeviceInstance](https://github.com/packerlschupfer/ESP32-IDeviceInstance) - Device instance interface
- FreeRTOS (included with ESP32 Arduino/IDF)
- Arduino framework for ESP32

### Optional
- [ESP32-Logger](https://github.com/packerlschupfer/ESP32-Logger) - Custom logging library (see [Logging Configuration](#logging-configuration))
...
├── TemperatureControlModule.h   # Temperature control module header (optional)
...
build_flags = 
    -DUSE_CUSTOM_LOGGER  ; Enable custom logger
    -DMB8ART_DEBUG       ; Optional: Enable debug logging
```
...
void setup() {
    ...
    // Register device for response routing
    modbus::ModbusRegistry::getInstance().registerDevice(0x01, &tempModule);
...
- **`examples/MB8ART-BasicExample/`** - Minimal single-file example, fewest dependencies
- **`examples/simple/`** - Quick start example with minimal setup
- **`examples/full/`** - Production-ready architecture with modular initialization

See [examples/README.md](examples/README.md) for details on which to use.

### FreeRTOS Task Manager Integration

The `examples/full/` project demonstrates advanced usage with FreeRTOS task management:
...
void setup() {
    ...
    if (xSemaphoreTake(sensorInstanceMutex, portMAX_DELAY)) {
        ...
        // Register device for response routing
        modbus::ModbusRegistry::getInstance().registerDevice(0x01, sensorInstance);
...
See the [`examples/`](examples/README.md) folder for complete projects demonstrating:
...
- **Memory Optimization**: Bit fields and structure packing for minimal RAM usage
