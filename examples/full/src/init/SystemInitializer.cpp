...
Result<void> SystemInitializer::initializeTasks() {
    ...
    const bool wdtOk = gTaskManager->initWatchdog(WATCHDOG_TIMEOUT_SECONDS, true);
    esp_log_level_set("task_wdt", ESP_LOG_WARN);
    if (!wdtOk) {
        // Not fatal: tasks still run, but they are no longer watchdog-protected.
        LOG_WARN(TAG, "Watchdog init failed - tasks will run unprotected");
    }
