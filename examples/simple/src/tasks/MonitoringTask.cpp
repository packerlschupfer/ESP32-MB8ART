...
void MonitoringTask::logTemperatureStatistics() {
    ...
    // Normalize all temperatures to tenths for statistics (common denominator)
    // PT1000+HIGH_RES values are divided by 10 to convert hundredths→tenths
    int16_t minTemp = 32767;
    int16_t maxTemp = -32768;
    int32_t sumTemp = 0;
    int validChannels = 0;

    // Calculate statistics - normalize all values to tenths
    for (int i = 0; i < MB8ART_NUM_CHANNELS; i++) {
        if (temperatureModule->wasSensorLastCommandSuccessful(i)) {
            int16_t rawTemp = temperatureModule->getSensorTemperature(i);
            int16_t divider = temperatureModule->getDataScaleDivider(
                IDeviceInstance::DeviceDataType::TEMPERATURE, i);

            // Normalize to tenths: if hundredths (divider=100), divide by 10
            int16_t tempInTenths = (divider == 100) ? (rawTemp / 10) : rawTemp;

            if (tempInTenths < minTemp) minTemp = tempInTenths;
            if (tempInTenths > maxTemp) maxTemp = tempInTenths;
            sumTemp += tempInTenths;
            validChannels++;
        }
    }
    ...
    if (validChannels > 0) {
        ...
        // All values normalized to tenths
