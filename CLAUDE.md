...
- **Temperature_t (int16_t)** storage - tenths of degrees (no float overhead)
- Unified hardware mapping with pointer bindings
...
The library uses unified mapping with pointer bindings for direct sensor data updates.

### Usage Example:
```cpp
// 1. Create MB8ART instance
MB8ART mb8art(3, "MB8ART");

// 2. Set hardware config (constexpr array in flash)
mb8art.setHardwareConfig(mb8art::DEFAULT_SENSOR_CONFIG.data());

// 3. Bind sensor pointers (library writes directly to your variables)
std::array<mb8art::SensorBinding, 8> bindings = {{
    ...
    {&sensorData.waterTank, &sensorData.isWaterTankValid},
...
// 5. Temperatures automatically update in your sensorData struct!
// No polling needed - library writes via bound pointers
```

### Temperature Type System:
- All temperatures stored as `int16_t` raw values
- **Per-channel scaling** based on input type AND resolution mode (register 76):
  - **Thermocouples**: Always tenths (÷10), not affected by resolution mode
  - **PT/RTD (PT100, PT1000, CU50, CU100)**: Follow resolution mode
    - LOW_RES: tenths (234 = 23.4°C, divider=10)
    - HIGH_RES: hundredths (2340 = 23.40°C, divider=100)
  - **Voltage/Current**: Assumed tenths (÷10)
- Use `getDataScaleDivider(DeviceDataType::TEMPERATURE, channel)` for per-channel scaling
- No float arithmetic - faster and deterministic
- See `HARDWARE.md` for complete register documentation
